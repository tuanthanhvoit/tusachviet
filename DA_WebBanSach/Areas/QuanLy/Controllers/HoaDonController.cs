...
namespace DA_WebBanSach.Areas.QuanLy.Controllers
{
    ...
    public class HoaDonController : Controller
    {
        ...
        public ActionResult Edit(int id = 0)
        {
            ...
            if (TempData["ErrorMaQL"]!=null)
            {
                ViewBag.Error = TempData["ErrorMaQL"].ToString();
            }
            return View(hoadon);
        }
        ...
        [HttpPost]
        public ActionResult Edit(HoaDon hoadon, string MaQuanLy, string NgayLap)
        {
            int ma;
            try
            {
                ma = int.Parse(MaQuanLy);
            }
            catch {
                ViewBag.Error = "Không đúng thông tin quản lý";
                return View(hoadon);
            }

            NguoiDung username = db.NguoiDungs.Find(ma);

            if (username != null)
            {
                if (!Roles.IsUserInRole(username.UserName,"Admin"))
                {
                    ViewBag.Error = "Không đúng thông tin quản lý";
                    return View(hoadon);
                }
        ...
        [HttpPost]
        public ActionResult EditChiTiet(int maHD, string MaQuanLy, string btnAction = "Sửa")
        {
            int ma;
            try
            {
            ...
            {
                TempData["ErrorMaQL"] = "Không đúng thông tin quản lý";
                ...
                foreach (var name in names)
                {
                    if (name.StartsWith("qty"))
                    {
                        ...
                        if (Request.Form[name].Length < 3)
                        {
                            ...
                            {
                                TempData["ErrorMaQL"] = "Số lượng không được nhập chữ";
                                return RedirectToAction("Edit", new { id = maHD });
                            }
                        }
                        else
                        {
                            TempData["ErrorMaQL"] = "Số lượng nhập quá lớn";
                            return RedirectToAction("Edit", new { id = maHD });
                        }
                    }
                }
